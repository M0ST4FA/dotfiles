...
-- Maximize / Fullscreen
hl.bind(mainMod .. " + F", hl.dsp.window.fullscreen({ action = "toggle", mode = 1, layout_aware = false }))
hl.bind(mainMod .. " + SHIFT + F", hl.dsp.window.fullscreen({ action = "toggle", mode = 0, layout_aware = false }))
...
hl.bind(mainMod .. " + SHIFT + W", function()
	hl.dispatch(hl.dsp.layout("focus float"))
end)
...
hl.bind(mainMod .. " + SHIFT + bracketright", hl.dsp.window.move({ direction = "u", group_aware = true }))
hl.bind(mainMod .. " + SHIFT + bracketleft", hl.dsp.window.move({ direction = "d", group_aware = true }))
hl.bind(mainMod .. " + bracketright", hl.dsp.window.move({ direction = "r", group_aware = true }))
hl.bind(mainMod .. " + bracketleft", hl.dsp.window.move({ direction = "l", group_aware = true }))
...
for i = 1, 9 do
	...
	hl.bind(mainMod .. " + CTRL + " .. i, function()
		hl.dispatch(hl.dsp.layout("movecoltoworkspace " .. i))
	end)
end
...
hl.bind(
	...
	hl.dsp.exec_cmd(
		"hyprctl -q keyword cursor:zoom_factor $(hyprctl getoption cursor:zoom_factor -j | jq '(.float * 0.80) | if . < 1 then 1 else . end')"
	),
	{ repeating = true }
)
